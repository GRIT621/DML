...
parser.add_argument('--name', default='Yahoo-100-40000-gelu',type=str,help='experiment name')
parser.add_argument('--data-path', default='./data', type=str, help='data path')
parser.add_argument('--save-path', default='../../../data2/lsj/mpl/9.24-checkpoint', type=str, help='save path')
...
parser.add_argument('--num_unlabeled', type=int, default=40000, help='number of unlabeled data')
...
parser.add_argument("--gpu_ids", type=list, default= [0,1], help="gpu-ids")
...
def train_loop(args, labeled_loader, unlabeled_loader, dev_loader,test_loader,
    ...
    for step in range(args.start_step, args.total_steps):
        ...
        with amp.autocast(enabled=args.amp):
            ...
            #t_loss_simi1 = 0.5 *(criteria(torch.log_softmax(t_logits_us,dim = -1) , torch.softmax(t_logits_uw, dim = -1))+ criteria(torch.log_softmax(t_logits_uw,dim = -1) , torch.softmax(t_logits_us, dim = -1)))
            ...
            # 加载pretrain model
                ...
                meta_net.update_params(1e-3, source_params=grads) # base 1e-3
       ...
       #     new_state = meta_net.state_dict()#['NTM.weight']
        ...
        if (step+1) % args.eval_step == 0:
            ...
            if args.local_rank in [-1, 0]:
                ...
                # if args.total_steps - step <= 5000:
                #     report, confusion,mif1,maf1,predict_all = inference_fn(args,test_model,test_loader)
                #     best_mif1 = mif1 if mif1 > best_mif1 else best_mif1
                #     best_maf1 = maf1 if maf1 > best_maf1 else best_maf1
                #     print("$$$",mif1,maf1,best_mif1,best_maf1)
            ...
            if args.dataset == "kg":
                ...
                print(report,mif1)
    if args.local_rank in [-1, 0]:
        args.writer.add_scalar("../../../data2/lsj/mpl/9.24-result/test_acc@1", args.best_top1)
...
def main():
    ...
    if args.local_rank in [-1, 0]:
        args.writer = SummaryWriter(f"../../../data2/lsj/mpl/9.24-result/{args.name}")
   ...
   #optimizer = optim.Adam([{'params':model.encoder.parameters()},{'params':model.fc.parameters(),'lr':0.001}], lr=learning_rate, weight_decay=weight_decay)
    ...
    if args.finetune:
        # del t_scaler, t_scheduler, t_optimizer, teacher_model, unlabeled_loader
        # del s_scaler, s_scheduler, s_optimizer
        finetune(args, labeled_loader, dev_loader, test_loader, student_model, criterion)
        return
...
def finetune(args, labeled_loader, dev_loader,test_loader, model, criterion):
    # train_sampler = RandomSampler if args.local_rank == -1 else DistributedSampler
    # labeled_loader = DataLoader(
    #     train_loader.dataset,
    #     sampler=train_sampler(train_loader.dataset),
    ...
    for epoch in range(args.finetune_epochs):
        ...
        if args.local_rank in [-1, 0]:
            args.writer.add_scalar("f-result/finetune_acc@1", args.best_top1)
            # wandb.log({"result/fintune_acc@1": args.best_top1})
    report, confusion, mif1, maf1, _ = inference_fn(args, model, test_loader)
    print("@@@@@@@@@@@@@@@@@@@@@2", report, confusion, mif1, maf1)
